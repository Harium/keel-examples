...
import com.harium.keel.filter.search.NoiseSearch;
...
public class FaceSampledMultiFilterStatic extends Application {
    ...
    private void reset(BufferedImage b) {
        ...
        blackPolygon.reset();
        Component blackComponent = quickFilter.filter(source, blackSampledFeature).get(0);
        blackPolygon = PolygonHelper.getPolygon(blackComponent);
        ...
        Component skinComponent = quickFilter.filter(source, skinFeature).get(0);
        skinPolygon = PolygonHelper.getPolygon(skinComponent);
    ...
    private void mergeAll() {
        ...
        PolygonHelper.mergePolygon(allPoints, skinPolygon);
        PolygonHelper.mergePolygon(allPoints, blackPolygon);

        merged.reset();
        Component mergedComponent = quickMergeFilter.filter(null, allPoints).get(0);
        merged = PolygonHelper.getPolygon(mergedComponent);
    }
